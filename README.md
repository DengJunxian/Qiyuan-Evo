...
启动后默认进入“图片演示”，直接浏览团队自演进和金融政策风洞的界面实录，点击“进入实验”打开自演进驾驶舱。原政策实验、历史验证、研判分析和成果展示入口作为金融政策风洞的业务能力保留。市场参与者的 StrategyGenome / personality evolution 仍属于 Layer B，不计作团队演化。

## 演示与运行截图

首页“图片演示”按 **团队自演进 → 金融政策风洞** 展示 16 张原图：8 张已完成的团队演进实验截图，以及原 Civitas 仓库的 8 张政策输入、市场推演、报告和历史验证截图。图片可直接打开大图，无需运行模型。

进入交互系统后，按 **实验总览 → 团队协作 → 自演进 → 三类实验** 讲解。“自演进”页可切换前后对比、提示词与工具、经验复用，研究工具内保留金融风洞、历史验证和技术资料。
...
团队截图来自已完成实验，原图及实验编号见 [截图清单](static/competition_gallery/manifest.json)。金融风洞图片沿用原仓库原图，来源版本与文件哈希见 [原项目截图清单](static/showcase_gallery/manifest.json)。[本次运行记录](docs/DEEPSEEK_DEMO_RUN.md)包含模型用量、失败修复与复现命令。
...
一级导航：**图片演示 → 实验总览 → 团队协作 → 自演进 → 三类实验**；金融政策风洞、历史验证和技术与复现收在“研究工具”中。

实验总览默认打开已完成的印花税案例：初轮遗漏了哪些验证、哪些成员参与、团队如何调整、两轮用时多少。点击“开始讲解：团队如何协作”，再点击“下一步：团队如何改进”，即可检查审查意见、配置变化和复评结果。原金融研究工具保留在侧栏与金融风洞页面。

实验总览用“完成验证 3/5 → 5/5 项”呈现进展，完整规则评分在两轮对照中。市场产出直接展示政策组与无政策组曲线、历史观测与回放偏差、干预方案风险与成本；零改善和增加的耗时照实保留。
...
| 路径 | 说明 |
| --- | --- |
| `app.py` | Streamlit 前端入口，默认图片演示，连接团队实验页面与原金融业务能力。 |
